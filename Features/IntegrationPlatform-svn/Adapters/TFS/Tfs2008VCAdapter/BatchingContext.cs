...
namespace Microsoft.TeamFoundation.Migration.Tfs2008VCAdapter
{
    ...
    public sealed class BatchingContext
    {
        ...
        public BatchingContext(Workspace workspace, ConfigurationService configurationService, ConflictManager conflictManager, int localWorkspaceVersion)
        {
            if (workspace == null)
            {
        ...
        private void reviseSourceName(BatchedItem renameItem)
        {
            ...
            {
                renameItem.Source = immediateParentPendedRename.Value + renameItem.Source.Substring(immediateParentPendedRename.Key.Length);
            }
        }
        ...
        private void pendRenames()
        {
            ...
            List<BatchedItem> renameShorterAdjustedList = new List<BatchedItem>();
        ...
        private void pendUndeletes()
        {
                ...
                foreach (BatchedItem batchedItem in batchedItems)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            {
                                // This is a TFS2008 only behavior - undelete the root folder will undelete sub items. But sub items won't
                                // show up in pended changes. They will be undeleted when the changes are commited. 
                                // So we will query the local file system, find the item and undo the undelete if there is no corresponding
                                // undelete in the original changeset.
                                foreach (string subItem in Directory.GetFileSystemEntries(m_workspace.GetLocalItemForServerItem(batchedItem.Source)))
                                {
                                    string serverPath = m_workspace.GetServerItemForLocalItem(subItem);
                                    if (m_undeletesToBePended.ContainsKey(serverPath))
                                    {
                                        m_undeletesToBePended[serverPath] = true;
                                    }
                                    else
                                    {
                                        subUndeleteToBeUndone.Add(serverPath);
                                    }
            ...
            {
                m_workspace.PendDelete(subUndeleteToBeUndone.ToArray());
            }
        }
            ...
            foreach (BatchedItem batchedItem in serverPaths)
            {
                ...
                while (VersionControlPath.Compare(currentPath, VersionControlPath.RootFolder) != 0)
                {
                    if (useSourcePath)
                    {
                        ...
                        {
                            batchedItem.AdjustSourceForParentRename(
                                VersionControlPath.Combine(m_pendedRenames[currentPath], (batchedItem.Source.Substring(currentPath.Length + 1))));
                            hasPendingChange = true;
                            break;
                        }
                    }
