...
namespace Microsoft.TeamFoundation.Migration.Shell.View
{
    ...
    public class ViewProgressCommand : ViewModelCommand<ShellController, ConfigurationModel>
    {
        #region Constructors
        /// <summary>
        /// Initializes a new instance of the <see cref="ViewProgressCommand&lt;TController, TModel&gt;"/> class.
        /// </summary>
        /// <param name="viewModel">The view model.</param>
        public ViewProgressCommand (ShellViewModel viewModel)
            : base (viewModel)
        {
        }
        #endregion
        ...
        {
            return ViewProgressCommand.CanOpen (viewModel);
        }

        internal static void Execute (ShellViewModel viewModel)
        {
            ViewProgressCommand.Open (viewModel);
        }
        #endregion
    }

    /// <summary>
    /// Provides a <see cref="CommandBinding"/> that delegates the <see cref="ApplicationCommands.Open"/> routed command to the <see cref="ViewProgressCommand&lt;TController, TModel&gt;"/>.
    /// </summary>
    /// <typeparam name="TController">The type of the controller.</typeparam>
    /// <typeparam name="TModel">The type of the model.</typeparam>
    public class ViewProgressCommandBinding : CommandBinding
    {
        #region Constructors
        /// <summary>
        /// Initializes a new instance of the <see cref="ViewProgressCommandBinding&lt;TController, TModel&gt;"/> class.
        /// </summary>
        /// <param name="viewModel">The view model.</param>
        public ViewProgressCommandBinding(ShellViewModel viewModel)
        {
            this.Command = ShellCommands.ViewProgress;

            this.CanExecute += delegate (object sender, CanExecuteRoutedEventArgs e)
            {
                e.CanExecute = ViewProgressCommand.CanExecute (viewModel, e.Parameter);
            };

            this.Executed += delegate (object sender, ExecutedRoutedEventArgs e)
            {
                ViewProgressCommand.Execute (viewModel);
            };
        }
        #endregion
    }
