...
namespace ServerDiff
{
    public class TfsServerDiff
    {
        ...
        private MigrationTestEnvironment m_testEnvironment;
        ...
        private bool Compare(Changeset targetChangeset, Changeset sourceChangeset)
        {
            ...
            foreach (Change targetChange in targetChanges.ToArray())
            {
                foreach (Change sourceChange in sourceChanges.ToArray())
                {
                    ...
                    if ((ChangeTypesMatch(targetChange.ChangeType, sourceChange.ChangeType))
                        && (NamesMatch(targetChange.Item.ServerItem, sourceChange.Item.ServerItem))
                        && (ContentsMatch(targetChange.Item.HashValue, sourceChange.Item.HashValue) || ((sourceChange.ChangeType & ChangeType.Delete) != 0)))
                    {
                        //Remove any changes from the list that matched in every way
                        targetChanges.Remove(targetChange);
                        sourceChanges.Remove(sourceChange);
        ...
        private bool ChangeTypesMatch(ChangeType targetChangeType, ChangeType sourceChangeType)
        {
            ...
            targetChangeType = targetChangeType & ~ChangeType.Encoding & ~ChangeType.Rollback;
            sourceChangeType = sourceChangeType & ~ChangeType.Encoding & ~ChangeType.Rollback;
            ...
            //Toolkit ignores Edit in Delete | Edit
            if (CheckChangeTypeSpecialCase(targetChangeType, sourceChangeType, (ChangeType.Delete | ChangeType.Edit), ChangeType.Delete))
            {
                return true;
            }

            // Merge|Add|SourceRename allows Add|SourceRename to map to Add only
            if (CheckChangeTypeSpecialCase(targetChangeType, sourceChangeType, (ChangeType.Add | ChangeType.SourceRename), 
                (ChangeType.Add | ChangeType.SourceRename | ChangeType.Merge) ))
            {
                return true;
            }
        ...
        {
            FilterItem item = m_testEnvironment.GetSourceFilterItem(filterPair);
