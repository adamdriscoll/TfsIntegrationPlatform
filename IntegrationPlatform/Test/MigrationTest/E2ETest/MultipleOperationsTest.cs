...
namespace TfsVCTest
{
    ...
    public class MultipleOperationsTest : TfsVCTestCaseBase
    {
        ...
        private void VerifyNoExtraMergeCandidates()
        {
            if (TargetTfsWorkspace != null)
            {
                // Fix up paths to replace the migration source path with the migration target path
                string migrationTargetSourceBranchPath = m_source.ServerPath.Replace(SourceAdapter.FilterString, TargetAdapter.FilterString);
                string migrationTargetTargetBranchPath = m_target.ServerPath.Replace(SourceAdapter.FilterString, TargetAdapter.FilterString);

                // Verify there are no merge candidates in either direction

                MergeCandidate[] migrationSourceMergeCandidates = 
                    SourceWorkspace.VersionControlServer.GetMergeCandidates(m_source.ServerPath, m_target.ServerPath, RecursionType.Full);
                MergeCandidate[] migrationTargetMergeCandidates = 
                    TargetTfsWorkspace.VersionControlServer.GetMergeCandidates(migrationTargetSourceBranchPath, migrationTargetTargetBranchPath, RecursionType.Full);
                Assert.IsTrue(migrationTargetMergeCandidates.Length <= migrationSourceMergeCandidates.Length, "Extra merge candidates on migration target's source branch");

                migrationSourceMergeCandidates = 
                    SourceWorkspace.VersionControlServer.GetMergeCandidates(m_target.ServerPath, m_source.ServerPath, RecursionType.Full);
                migrationTargetMergeCandidates = 
                    TargetTfsWorkspace.VersionControlServer.GetMergeCandidates(migrationTargetTargetBranchPath, migrationTargetSourceBranchPath, RecursionType.Full);
                Assert.IsTrue(migrationTargetMergeCandidates.Length <= migrationSourceMergeCandidates.Length, "Extra merge candidates on migration target's target branch");
            }
        }
        ...
        ///<summary>
        ///Scenario: Rename source/file1.txt to source/file2.txt; Checkin; Merge source to target, rename target/file2.txt to target/file3.xt; Checkin
        ///Expected Result: Mapped to the correct sequence of actions
        ///</summary>
        [TestMethod(), Priority(2), Owner("billbar")]
        [Description("Rename source/file1.txt to source/file2.txt; Checkin; Merge source to target & rename target/file2.txt to target/file3.xt; Checkin")]
        public void RenameMergeRenameTest()
        {
            RenameMergeRenameScenario(false);
        }

        ///<summary>
        ///Scenario: Rename source/file1.txt to source/file2.txt; Checkin; Merge source to target, rename target/file2.txt to target/file3.xt, edit target/file3.x; Checkin
        ///Expected Result: Mapped to the correct sequence of actions
        ///</summary>
        [TestMethod(), Priority(2), Owner("billbar")]
        [Description("Rename source/file1.txt to source/file2.txt; Checkin; Merge source to target & rename target/file2.txt to target/file3.xt, edit target/file3.x; Checkin")]
        public void RenameMergeRenameEditTest()
        {
            RenameMergeRenameScenario(true);
        }
