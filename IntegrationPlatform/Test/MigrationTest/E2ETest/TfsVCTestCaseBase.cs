...
namespace TfsVCTest
{
    public abstract class TfsVCTestCaseBase : VCMigrationTestCaseBase
    {
        ...
        protected Workspace TargetTfsWorkspace
        {
            get
            {
                ITfsVCTestCaseAdapter tfsVCTestCaseAdapter = TargetAdapter as ITfsVCTestCaseAdapter;
                if (tfsVCTestCaseAdapter == null)
                {
                    return null;
                }
                else
                {
                    return tfsVCTestCaseAdapter.Workspace;
                }
            }
        }
        ...
        protected void VerifySnapshotMigration(int snapShotChangesetId)
        {
            if (IsTfsToTfsMigration())
            {
                TfsServerDiff diff = new TfsServerDiff(new Guid(base.VCSession.SessionUniqueId), TestEnvironment, true, true);
                diff.QueryHistory();
                diff.RemoveSimilarHistory();
        ...
        {
            return TestEnvironment.TargetEndPoint.IsTfsAdapter && TestEnvironment.SourceEndPoint.IsTfsAdapter;
        }
    }
}
