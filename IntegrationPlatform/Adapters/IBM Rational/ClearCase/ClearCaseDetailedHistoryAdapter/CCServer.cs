    ...
    public class ClearCaseServer
    {
        ...
        public void UndoCheckoutRecursive(string path)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(lsCheckoutCmdOutput))
                {
                    checkoutFileList = CCTextParser.ParseLSOutput(lsCheckoutCmdOutput);
                    if (checkoutFileList != null)
                    {
        ...
        public List<string> QueryViewPrivateFiles(string viewName)
        {
            ...
            try
            {
                foreach(string vob in VobList)
                {
                    ...
                    string lsCommandOutput = ExecuteClearToolCommand(lsPrivateCmd);
                    if (!string.IsNullOrEmpty(lsCommandOutput))
                    {
                        List<string> fileList = CCTextParser.ParseLSOutput(lsCommandOutput);
                        if (fileList != null)
                        {
                            viewPrivateFileList.AddRange(fileList);
                        }                           
                    }
                }
            }
            ...
            bool writeHistoryRecordsFound)
        {
            ...
            foreach (MappingEntry mapping in filters)
            {
                ...
                try
                {
                    TraceManager.TraceInformation("ClearCase history command: '{0}'", historyCmd);
                ...
                catch (COMException ce)
                {
                    ...
                    {
                        TraceManager.TraceWarning("ClearCase history command failed: {0} : {1}", comResult.ToString(), ce.Message);
                        continue;
                    }
                    throw;
                }
                ...
                else
                {
                    ...
                    {  
                        historyRecord = CCHistoryRecord.CreateInstance(historyRow);
