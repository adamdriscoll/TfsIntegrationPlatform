...
namespace Microsoft.TeamFoundation.Migration.ClearCaseDetailedHistoryAdapter
{
    ...
    public class CCHistoryRecord : IComparable
    {
        ...
        public static CCHistoryRecord CreateInstance(string historyRow)
        {
            ...
            {
                TraceManager.TraceWarning("Invalid history record - {0}", historyRow);
                return null;
            }
            ...
            if (historyColumns.Length < 8)
            {
                TraceManager.TraceWarning("Wrong numbers of columns in history record - '{0}'", historyRow);
                return null;
            }
            string[] strEventId =
            ...
            {
                TraceManager.TraceWarning("Event id missing in history record - '{0}'", historyRow);
                return null;
            }
            ...
            {
                TraceManager.TraceWarning("Invalid event id in history record - '{0}'", historyRow);
                return null;
            }
            ...
            {
                TraceManager.TraceWarning("Invalid version time in history record - '{0}'", historyRow);
                return null;
            }
