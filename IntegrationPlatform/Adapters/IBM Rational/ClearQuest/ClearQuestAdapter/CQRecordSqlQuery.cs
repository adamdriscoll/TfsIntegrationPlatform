...
@"SELECT dbid 
  FROM {0} 
  WHERE (dbid in (SELECT DISTINCT entity_dbid FROM history WHERE action_timestamp > {1}{2}{3}))";
...
@"SELECT dbid 
  ...
  WHERE (dbid in (SELECT DISTINCT entity_dbid FROM history))";
            ...
            string hwmStr)
        {
            ...
            {
                m_queryStr = UtilityMethods.Format(QueryBase, recordFilters[0].SelectFromTable, m_queryTimeDelimiter, hwmStr, m_queryTimeDelimiter);
            }
