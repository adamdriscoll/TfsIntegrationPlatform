    ...
    internal class ClearQuestAnalysisProvider : IAnalysisProvider
    {
            ...
            string hwmTime)
        {
            CQRecordQueryBase recordQuery =
                CQRecordQueryFactory.CreateQuery(m_userSession, filter, hwmTime, this);
            ...
            OAdEntity aRecord)
        {
            ...
            string lastHistoryVersion = null;
            ...
            for (int histFldIndex = 0; histFldIndex < historyFldCount; histFldIndex++)
            {
                ...
                TraceManager.TraceVerbose("ClearQuest Adapter ComputeDeltaPerRecord: {0}:{1}; historyCount={2}; startHistIndex={3}",
                    recordEntDefName, recordDispName, historyCount, startHistIndex);
                ...
                for (int histIndex = startHistIndex; histIndex < historyCount; histIndex++)
                {
                    ...
                    if (TranslationService.IsSyncGeneratedItemVersion(ClearQuestRecordItem.GetMigrationRecordId(recordEntDefName, recordDispName),
                                                                      ...
                                                                      m_configurationService.SourceId))
                    {
                        TraceManager.TraceVerbose("ClearQuest Adapter: Skipping version {0} of item {1}:{2} because it is a sync generated version",
                            migrationItem.MigrationItemVersion, recordEntDefName, recordDispName);
                        continue;
                    }
                    TraceManager.TraceVerbose("ClearQuest Adapter: Generating delta for version {0} of item {1}:{2}",
                        migrationItem.MigrationItemVersion, recordEntDefName, recordDispName);
                    ...
                    switch (actionType)
                    {
                        ...
                        case CQConstants.ACTION_MODIFY:
                            ...
                            break;
                        case CQConstants.ACTION_CHANGE_STATE:
                            recordContentIsModified = true;
                            break;
                ...
                lastHistoryVersion = CQHistoryMigrationItem.CreateHistoryItemVersion(historyFieldName, historyCount - 1);
            }
            ...
            {
                TraceManager.TraceVerbose("ClearQuest Adapter: No non-sync delta versions found for record {0}", recordDispName);
            }
                 ...
                 aAttachmentFieldsIndex++)
            {
                ...
                int attachmentsCount = CQWrapper.AttachmentsCount(attachments);
                if (attachmentsCount > 0)
                {
                    TraceManager.TraceVerbose("ClearQuest Adapter: Generating actions for {0} attachments on record {1}", attachmentsCount, recordDispName);
                    for (int attachmentIndex = 0;
                         attachmentIndex < attachmentsCount;
                         attachmentIndex++)
                    {
                        ...
                        attachmentItem.CreateChangeAction(changeGroup, lastHistoryVersion);
