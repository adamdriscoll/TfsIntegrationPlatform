...
{
    public class Tfs2010LinkProvider : TfsLinkingProviderBase, IForceSyncLinkProvider
    {
        ...
        /// <summary>
        /// Generate the link change groups for force sync items
        /// </summary>
        public void GenerateLinkDeltaForForceSync(IEnumerable<string> itemIdsToForceSync)
        {
            IEnumerable<TfsMigrationWorkItem> itemsToForcSync = m_migrationSource.WorkItemStore.GetItems(itemIdsToForceSync);

            foreach (TfsMigrationWorkItem itemToForceSync in itemsToForcSync)
            {
                CreateLinkChangeGroupsForForceSyncItem(itemToForceSync);
            }
        }
