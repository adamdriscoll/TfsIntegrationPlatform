...
namespace Microsoft.TeamFoundation.Migration.Tfs2010WitAdapter
{
    ...
    public partial class TfsMigrationWorkItemStore : IComparer<Identity>
    {
        ...
        /// <param name="highWatermark">Highwater mark object</param>
        /// <returns>Collection of items</returns>
        public IEnumerable<TfsMigrationWorkItem> GetItems(IEnumerable<string> workItemIds)
        {
            // Create condition
            ...
            foreach (string workItemId in workItemIds)
            {
                ...
                itemAdded = true;
                queryBuilder.Append(workItemId);
            }
            queryBuilder.Append(')');
            ...
            DateTime excludeItemsChangedBeforeTime)
        {
            ...
            foreach (WorkItemLinkChange linkChange in WorkItemServer.GetWorkItemLinkChanges(Guid.NewGuid().ToString(), linkChangeIdHighWaterMark))
            {
                ...
                */
                ...
                if (linkChange.ChangedDate.ToUniversalTime() < excludeItemsChangedBeforeTime)
                {
        ...
        internal bool IsSourceWorkItemInBacklog(
            ...
            IMigrationAction action)
        {
            if (action.ChangeGroup.IsForcedSync)
            {
                // Ignore backlog for force sync items
                return false;
            }
            ...
            IMigrationAction action)
        {
            ...
            bool changeEditToAdd = false;
            if (action.Action == WellKnownChangeActionId.Edit)
            {
                if (action.ChangeGroup.IsForcedSync)
                {
                    // In the force sync case Edit actions are always generated in case the target exists, but it might not so check here
                    try
                    {
                        int targetWorkItemId = FindTargetWorkItemId(action, true);
                    }
                    catch (HistoryNotFoundException)
                    {
                        // Target work item not found to Edit; Add instead
                        changeEditToAdd = true;
                    }
                }
            ...
            if (action.Action == WellKnownChangeActionId.Add || changeEditToAdd)
            {
        ...
        protected string GenerateMigrationHistoryComment(IMigrationAction action)
        {
            ...
            string sourceWorkItemRevision = GetSourceWorkItemRevision(action);
            string forceSyncIndicator = action.ChangeGroup.IsForcedSync ? " via forced sync" : string.Empty;
            string sourceWorkItemComment = string.Format("{0} (rev {1}){2}", sourceWorkItemId, sourceWorkItemRevision, forceSyncIndicator);
            return commentDecorationService.GetChangeGroupCommentSuffix(sourceWorkItemComment);
        }
            ...
            string comment)
        {
            ...
            foreach (Attachment a in item.Attachments)
            {
                    ...
                    && string.Equals(originalName, a.Name, StringComparison.InvariantCulture))
                {
                    ...
                    var fileAttachment = new TfsMigrationFileAttachment(a, m_core.TfsTPC.Uri.AbsoluteUri);
