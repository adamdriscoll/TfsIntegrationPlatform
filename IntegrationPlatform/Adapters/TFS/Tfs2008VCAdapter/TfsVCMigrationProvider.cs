    ...
    public class TfsVCMigrationProvider : IMigrationProvider
    {
            ...
            HashSet<string> skippedActions, bool autoResolve)
        {
                    ...
                    else
                    {
                        switch (sourceChangeType)
                        {
                            ...
                            case ChangeType.Delete | ChangeType.Edit:
                                changeTypeMatch = (targetChangeType == ChangeType.Delete); // Rollback|Delete|Edit map to Delete only.
                                break;
                            ...
                            case ChangeType.Merge | ChangeType.Undelete:
                                changeTypeMatch = (targetChangeType == ChangeType.Undelete // Undelete is from a version migrated, but merge is from a verion not migrated
                                                || targetChangeType == (ChangeType.Undelete | ChangeType.Edit) // Undelete is from a version migrated, but merge is from a verion not migrated. Edit is pended for Undelete always.
            ...
            foreach (KeyValuePair<string, Change> remaingChange in sourceChanges)
            {
                ...
                if ((remaingChange.Value.ChangeType 
                    & ~ChangeType.Encoding 
                    & ~ChangeType.Lock
                    & ~ChangeType.Merge
                    & ~ChangeType.None
                    & ~(ChangeType) 1024 // Rollback
                    & ~(ChangeType) 2048 // SourceRename
                    )== 0)
                {
            ...
            if (!group.Comment.Contains("Initial Check-in as snapshot at changeset") && m_sourceSideIsTfs)
            {
             ...
             if (changes.Length > 0)
             {
                 ...
                 catch (CheckinException checkinException)
                 {
                     ...
                     List<MigrationAction> retActions;
                     MigrationConflict checkinFailureConflict = TfsCheckinFailureConflictType.CreateConflict(group.Name, GetConflictDetailsFromTfsException(checkinException));
