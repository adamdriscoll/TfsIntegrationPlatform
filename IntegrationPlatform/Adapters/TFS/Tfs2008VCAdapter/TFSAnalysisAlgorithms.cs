...
namespace Microsoft.TeamFoundation.Migration.Tfs2008VCAdapter
{
    ...
    internal abstract class TfsAnalysisAlgorithms
    {
        ...
        {
            changeType &= ~(ChangeType)2048; // SourceRename
            changeType &= ~ChangeType.Lock;
            changeType &= ~ChangeType.Encoding;
            changeType &= ~(ChangeType)1024; // We just skip rollback changetype, other combined change type will be migrated. 
            ...
            if (previous != null)
            {
                switch (Provider.IsPathMapped(previous.ServerItem, previous.ChangesetId))
                {
                    case MappingResult.Mapped:
                        ...
                        // Always pend an edit for undelete of a file
        ...
        private void processBatchedMerges(ChangeGroup group)
        {
            ...
            foreach (KeyValuePair<string, Change> remainingMerge in m_batchedMerges)
            {
                ...
                else
                {
                    ...
                    {
                        TraceManager.TraceWarning(
                            "Merge item '{0}' not found by QueryMergesWithDetails so can't be migrated; it may have been destroyed.",
                            remainingMerge.Key);
                        continue;
                    }
                    catch (VersionControlException vce)
        ...
        private void processBatchedBranches(ChangeGroup group)
        {
            ...
            foreach (string branchRoot in branchRoots)
            {
                ...
                {
                    TraceManager.TraceWarning(
                        "Branch root '{0}' not found by QueryMergesWithDetails so can't be migrated; it may have been destroyed.",
                        branchRoot);
                    continue;
                }
                catch (VersionControlException vce)
    ...
    internal class Tfs2008AnalysisAlgorithms : TfsAnalysisAlgorithms
    {
        ...
        internal override void BuildChangeTranslators()
        {
            ...
            addHandler((ChangeType)1024, None); // Rollback on TFS2010
            addHandler((ChangeType)2048, None); // SourceRename on TFS2010
        }
    }
}
