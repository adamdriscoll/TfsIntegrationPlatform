...
namespace Microsoft.TeamFoundation.Migration.Tfs2010VCAdapter
{
    ...
    internal abstract class TfsAnalysisAlgorithms
    {
        ...
        internal static ChangeType stripChangeType(ChangeType changeType)
        {
            ...
            // Certain version specific change types may not be handled yet so are ignored; strip these off as well
            if ((VersionSpecificUtils.ChangeTypesToIgnore != ChangeType.None) && ((int)(changeType & VersionSpecificUtils.ChangeTypesToIgnore) > 0))
            {
                changeType &= ~VersionSpecificUtils.ChangeTypesToIgnore;
                TraceManager.TraceWarning("Ignoring unsupported VC ChangeTypes: " + VersionSpecificUtils.ChangeTypesToIgnore.ToString());
            }
        ...
        /// <summary>
        /// None is not a valid change type, but we can get here if the only ChangeTypes for a TFS Change are ones in VersionSpecificUtils.ChangeTypesToIgnore
        /// In this case, return without creating a migration action
        /// </summary>
        /// <param name="change">The change being processed</param>
        /// <param name="group">The change grouing this change is to be a part of</param>
        ...
        private void processBatchedMerges(ChangeGroup group)
        {
            ...
            foreach (string mergeRoot in mergeRoots)
            {
                ...
                {
                    // MergeRoot may have been destroyed; can't migrate it
                    TraceManager.TraceWarning(
                        "Merge root {0} not found by QueryMergesWithDetails so can't be migrated; it may have been destroyed.",
                        mergeRoot);
                    continue;
                }
