...
namespace Microsoft.TeamFoundation.Migration.Tfs2010VCAdapter
{
    ...
    public static class TfsUtil
    {
        ...
        public static bool IsTfs2010Server(TfsTeamProjectCollection tfsServer)
        {
            // Check ISCCProvider3 service that hosts new TFS2010 only features.
            ILocationService locationService = tfsServer.GetService<ILocationService>();
            string ISCCProvider3Url = locationService.LocationForCurrentConnection(RepositoryConstants.ISCCProvider3, RepositoryConstants.ISCCProvider3ServiceIdentifier);
            if (string.IsNullOrEmpty(ISCCProvider3Url))
            {
                return false;
            }
            return true;
        ...
        public static bool isChildItemOfRename(BatchedItem item, BatchedItem parentItem, out bool childIsAlsoExplicitRename)
        {
            childIsAlsoExplicitRename = false;
            ...
            if ((item.Action == parentItem.Action) && (VersionControlPath.IsSubItem(item.Target, parentItem.Target)))
            {
                if ((item.Action == WellKnownChangeActionId.Rename) && (VersionControlPath.IsSubItem(item.Source, parentItem.Source)))
                {
                    ...
                    {
                        childIsAlsoExplicitRename = true;
                    }
                    return true;
                }
            }
