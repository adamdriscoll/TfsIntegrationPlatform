...
namespace Microsoft.TeamFoundation.Migration.Tfs2010VCAdapter
{
    public class TfsVCMigrationProvider : IMigrationProvider
    {
        ...
        {
            TfsTeamProjectCollection tfsServer = TfsTeamProjectCollectionFactory.GetTeamProjectCollection(new Uri(m_configurationService.ServerUrl));
            if (!TfsUtil.IsTfs2010Server(tfsServer))
            {
                throw new MigrationException(string.Format("The target server {0} is not a TFS2010 server", m_configurationService.ServerUrl));
            }
        ...
        public virtual void RegisterConflictTypes(ConflictManager conflictManager)
        {
            ...
            m_conflictManagementService.RegisterConflictType(new TfsCheckinFailureConflictType());
        ...
        public ConversionResult ProcessChangeGroup(ChangeGroup group)
        {
            ...
            try
            {
                if (Workspace == null)
                {
                    throw new VersionControlMigrationException("Cannot process ChangeGroup because the TFS Workspace was not initialized due to an earlier error");
                }
                ...
                {
                    // Checkin returned false rather than raising a TfsCheckinFailedConflict so we'll treat this as a Runtime Error
        ...
        protected virtual string GetChangeComment(ChangeGroup group)
        {
            ...
            string migrationComment = m_commentDecorationService.GetChangeGroupCommentSuffix(group.Name, group.ChangeTimeUtc);
            ...
            return group.Comment + Environment.NewLine + migrationComment;
        }
        ...
        protected virtual string GetChangeOwner(ChangeGroup group)
        {
            try
            {
                var components = group.Owner.Split('\\');
                if (components.Length > 1)
                {
                    var domain = components[0];
                    var userName = components[1];

                    DirectoryEntry de = new DirectoryEntry("WinNT://" + domain + "/" + userName);
        ...
        private bool codeReview(IEnumerable<PendingChange> pendChanges, int changeset, HashSet<string> implicitRenames, HashSet<string> implicitAdds, 
            HashSet<string> skippedActions, out int changeCount, bool autoResolve)
        {
            ...
            foreach (Change sourceChange in m_sourceTfsClient.GetChangeset(changeset).Changes)
            {
                ...
                {
                    ChangeType supportedChangeTypes = sourceChange.ChangeType & VersionSpecificUtils.SupportedChangeTypes;
                    if ((int)supportedChangeTypes == 0 || supportedChangeTypes == ChangeType.None)
                    {
                        // The source change only contained ChangeTypes that are not supported by this rev; ignore it
                        continue;
                    }
            ...
            foreach (PendingChange pendingChange in pendChanges)
            {
                ...
                if (sourceChanges.ContainsKey(pendingChange.ServerItem))
                {
                    ChangeType sourceChangeType = sourceChanges[pendingChange.ServerItem].ChangeType & ~ChangeType.Encoding & ~ChangeType.Lock 
                        & ~ChangeType.SourceRename & ~ChangeType.Rollback;

                    // The TFS revision on the source side may support change types not support by the rev for this adapter
                    // so apply a mask that removes any change type not supported by this rev
                    sourceChangeType &= VersionSpecificUtils.SupportedChangeTypes;
                    ...
                    else if ((sourceChangeType | ChangeType.Edit) == targetChangeType)
                    {
                        // Target has an extra Edit. 
                        // For some change actions (s.a. Undelete), we append an extra edit to make sure content is the same. 
                        changeTypeMatch = true;
                    }
                    else
                    {
                        switch (sourceChangeType)
                        {
                            ...
                            case ChangeType.Branch | ChangeType.Edit | ChangeType.Merge:
                                ...
                                break;
                            case ChangeType.Delete | ChangeType.Edit:
                                changeTypeMatch = (targetChangeType == ChangeType.Delete); // Rollback|Delete|Edit mapped to Delete.
                                break;
                            case ChangeType.Delete | ChangeType.Merge:
                                // Some source adapters map Merge|Delete to Delete; newer versions of TFS VC Adapters map Merge|Delete to Merge (accept either)
                                changeTypeMatch = (targetChangeType == ChangeType.Delete) || (targetChangeType == ChangeType.Merge); 
                                break;
                            ...
                            case ChangeType.Add:
                                changeTypeMatch = (targetChangeType == (ChangeType.Add | ChangeType.Edit)
                                                  | (targetChangeType == ChangeType.Edit)); // Change to Add on missing for Edit
                                break;
                            case ChangeType.Add | ChangeType.Merge:
                            ...
                            case ChangeType.Merge | ChangeType.Undelete:
                                changeTypeMatch = (targetChangeType == ChangeType.Undelete // Undelete is from a version migrated, but merge is from a verion not migrated
                                                || targetChangeType == (ChangeType.Undelete | ChangeType.Edit)
                            ...
                            case ChangeType.Merge | ChangeType.Rename:
                                changeTypeMatch = (targetChangeType == ChangeType.Rename // Merge from item or version not migrated. 
                                                || targetChangeType == ChangeType.Merge); // In TFS2010, Merge|rename| can be mapped to Merge if the rename target is also a rename source
                                break;
                            case ChangeType.Merge | ChangeType.Rename | ChangeType.Undelete:
                                changeTypeMatch = (targetChangeType == ChangeType.Add
                                                || targetChangeType == (ChangeType.Edit | ChangeType.Rename | ChangeType.Undelete)
                            ...
                            case ChangeType.Undelete:
                                changeTypeMatch = (targetChangeType == ChangeType.Add // Undelete from before the snapshot
                                                || targetChangeType == ChangeType.Edit // Undelete from before the snapshot
                                                || targetChangeType == (ChangeType.Add | ChangeType.Edit) // Undelete from before the snapshot
                                                || targetChangeType == (ChangeType.Undelete | ChangeType.Edit)); // Undelete mapped to Undelete|Edit.
                                break;
                            default:
                                break;
                        }
            ...
            foreach (KeyValuePair<string, Change> remaingChange in sourceChanges)
            {
                if (skippedActions.Contains(remaingChange.Key))
                {
                ...
                else if ((remaingChange.Value.ChangeType 
                    & ~ChangeType.Encoding 
                    & ~ChangeType.Lock 
                    ...
                    & ~ChangeType.Merge )== 0)
                {
                    // Skip non content change combinations.
                    continue;
                }
                ...
                else if (((remaingChange.Value.ChangeType & ~ChangeType.Encoding & ~ChangeType.Lock
                                                          & ~ChangeType.SourceRename & ~ChangeType.Rollback & ~ChangeType.Merge) == ChangeType.Rename)
            ...
            if (localDiffResults.Count > 0)
            {
                if (autoResolve)
                {
                    if (changeCount == 0)
                    {
                        changeCount = localDiffResults.Count;
                    }
                ...
                else
                {
                    ...
                    {
                        foreach (KeyValuePair<string, Change> errorItem in localDiffResults)
                        {
                            if (errorItem.Value == null)
                            {
                                // Item is changed on target server only.
                                writer.WriteLine(errorItem.Key);
                            }
                            else
                            {
                                writer.WriteLine(string.Format("{0} {1}", errorItem.Key, errorItem.Value.ChangeType));
                            }
                        }
                    }
                    return false;
        ...
        /// <summary>
        /// This checkin method is TFS2010 specific. 
        /// It contains TFS2010 OM method workspace.GetPendingChangeEnumerable(). 
        /// </summary>
        /// <param name="group"></param>
        /// <param name="implicitRenames"></param>
        /// <param name="implicitAdds"></param>
        /// <param name="changesetId"></param>
        /// <returns></returns>
        ...
        {
            Exception exceptionForCheckinFailureConflict = null;
            changesetId = -1;
            IEnumerable<PendingChange> changes = Workspace.GetPendingChangesEnumerable();
            int changeCount = -1;
            ...
            if (!group.Comment.Contains("Initial Check-in as snapshot at changeset") && m_sourceSideIsTfs)
            {
                if (!codeReview(changes, int.Parse(group.Name), implicitRenames, implicitAdds, skippedActions, out changeCount, false))
                {
                    ...
                    codeReview(changes, int.Parse(group.Name), implicitRenames, implicitAdds, skippedActions, out changeCount, true);
                    changes = Workspace.GetPendingChangesEnumerable();
                }
            }
            else
            {
                changeCount = Workspace.GetPendingChanges().Length;
            }

            // Code review will iterate the IEnumerable changes and get the count. 
            // If the count is changed from -1 to 0, no changes are pended.
            if (changeCount == 0)
            {
            ...
            {
                checkinParameters.ValidateAuthor = false;
            }
            checkinParameters.OverrideGatedCheckIn = true; // Override gated checkin.
            checkinParameters.AllowUnchangedContent = true; // tf checkin /force

            bool retryWithDefaultOwner = false;

            TraceManager.TraceInformation("Checking in {0} items, owner {1}", changeCount, checkinParameters.Author);

            // Used to allow time to force check-in conflict manually
            if (Environment.GetEnvironmentVariable("TFSIP_BREAK_BEFORE_TFS_CHECKIN") != null)
            {
                Debug.Fail(String.Format("About to call TFS Workspace.CheckIn() to checkin {0} items", changeCount));
            }
            ...
            while (true)
            {
                ...
                catch (IdentityNotFoundException inf)
                {
                    ...
                    {
                        // We've already tried check-in with default owner. 
                        // Just break out.
                        exceptionForCheckinFailureConflict = inf;
                    }
                }
                catch (ChangesetAuthorMustBeNonGroupException ca)
                {
                    ...
                    {
                        exceptionForCheckinFailureConflict = ca;
                    }
                }
                ...
                {
                    if (checkinException.Message != null)
                    {
                        TraceManager.TraceWarning("Checkin failed - {0}", checkinException.Message);
                    }

                    if ((checkinException.Conflicts != null) && (checkinException.Conflicts.Length > 0))
                    {
                        foreach (CheckinConflict conflict in checkinException.Conflicts)
                        {
                            TraceManager.TraceWarning("Checkin failed - {0}", conflict.Message);
                        }
                    }
                    exceptionForCheckinFailureConflict = checkinException;
                }
                catch (Exception ge)
                {
                    if (ge.Message.Equals("The array must contain at least one element.\r\nParameter name: checkinParameters.PendingChanges"))
                    {
                        TraceManager.TraceInformation("After processing there were 0 pending changes for group id {0}", group.ChangeGroupId);
                        changesetId = 0;
                        break;
                    ...
                    else
                    {
                        ...
                        {
                            // We've already tried check-in use default owner. 
                            // Break out.
                            break;
                        }
                    }
                ...
                if (exceptionForCheckinFailureConflict != null)
                {
                    List<MigrationAction> retActions;
                    MigrationConflict checkinFailureConflict = TfsCheckinFailureConflictType.CreateConflict(group.Name, GetConflictDetailsFromTfsException(exceptionForCheckinFailureConflict));
                    ConflictResolutionResult resolutionResult = m_conflictManagementService.TryResolveNewConflict(group.SourceId, checkinFailureConflict, out retActions);
                    TraceManager.TraceWarning("Check in failed. Please resolve the checkinfailure conflict so the migration can resume");
                    throw new MigrationUnresolvedConflictException(checkinFailureConflict);
                }

                if (retryWithDefaultOwner)
                {
                    checkinParameters.Comment = m_commentDecorationService.AddToChangeGroupCommentSuffix(checkinParameters.Comment,
                        string.Format(
                            TfsVCAdapterResource.Culture,
                            TfsVCAdapterResource.AuthorFallbackNote,
                            checkinParameters.Author));
        ...
        private string GetConflictDetailsFromTfsException(Exception tfsExceptionThrownByCheckin)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "Check in to TFS server at '{0}' failed", m_workspace.VersionControlServer.TeamProjectCollection.Uri));
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "{0}: {1}.", tfsExceptionThrownByCheckin.GetType().ToString(), tfsExceptionThrownByCheckin.Message));

            CheckinException checkinException = tfsExceptionThrownByCheckin as CheckinException;
            if (checkinException != null)
            {
                if (checkinException.Conflicts.Length > 0)
                {
                    sb.AppendLine("Specific error(s) that prevented the TFS checkin:");
                }
                foreach (CheckinConflict conflict in checkinException.Conflicts)
                {
                    sb.AppendLine(conflict.Message);
                }
            }
        ...
        protected virtual Workspace CreateWorkspace()
        {
            ...
            Workstation.Current.UpdateWorkspaceInfoCache(m_tfsClient, m_tfsClient.AuthorizedUser);
            ...
            catch (WorkspaceNotFoundException)
            {
                ...
                ws = VersionSpecificUtils.CreateWorkspace(
                    m_tfsClient,
                    m_configurationService.Workspace,
                    m_tfsClient.AuthorizedUser,
            ...
            foreach (MappingEntry mapping in m_configurationService.Filters)
            {
                ...
                if (allowedLocalPathLength > 0)
                {
                    localPath = Path.Combine(m_configurationService.WorkspaceRoot,
                        mapping.Path.Substring(2).Replace('/', '\\').Substring(m_configurationService.WorkspaceRoot.Length-1).TrimStart(new char[] { '\\' }));
                ...
                TraceManager.TraceInformation("Added TFS workspace mapping from server path '{0}' to local path '{1}'", mapping.Path, localPath);
            }

            // Apply the cloaks after the Active mappings since the cloaked paths need to be underneath some mapped path
            foreach (string cloakPath in cloakPaths)
            {
                ws.Cloak(cloakPath);
                TraceManager.TraceInformation("Added TFS workspace cloak of server path '{0}", cloakPath);
            }
