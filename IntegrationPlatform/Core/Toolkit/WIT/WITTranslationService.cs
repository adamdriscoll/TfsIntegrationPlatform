...
namespace Microsoft.TeamFoundation.Migration.Toolkit.WIT
{
    public class WITTranslationService : TranslationServiceBase
    {
        ...
        public override void Translate(IMigrationAction action, Guid migrationSourceIdOfChangeGroup)
        {
            ...
            if (!action.ChangeGroup.IsForcedSync && IsSyncGeneratedAction(action, migrationSourceIdOfChangeGroup))
            {
        ...
        public override bool IsSyncGeneratedAction(IMigrationAction action, Guid migrationSourceIdOfChangeGroup)
        {
            ...
            bool isSyncGeneratedAction = IsSyncGeneratedItemVersion(sourceItemId, sourceItemRevision, migrationSourceIdOfChangeGroup);
            stopwatch.Stop();
            if (stopwatch.ElapsedMilliseconds > 100)
            {
                TraceManager.TraceInformation(String.Format("WITTranslationService: IsSyncGeneratedItemVersion completed in {0} ms", stopwatch.ElapsedMilliseconds));
            }
            return isSyncGeneratedAction;
        }
