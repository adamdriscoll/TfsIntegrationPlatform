...
namespace Microsoft.TeamFoundation.Migration.Toolkit.WIT
{
    ...
    class WITBasicConflictAnalysisService : IConflictAnalysisService
    {
        ...
        public void Analyze()
        {
            ...
            try
            {
                TraceManager.TraceInformation("Starting FlattenDeltaTable for source");
                FlattenDeltaTable(m_mappedWorkItemsInSourceDeltaTable, m_mappedWorkItemAttachmentUpdatesInSource, true);
                TraceManager.TraceInformation("Starting FlattenDeltaTable for target");
                FlattenDeltaTable(m_mappedWorkItemsInTargetDeltaTable, m_mappedWorkItemAttachmentUpdatesInTarget, false);
                TraceManager.TraceInformation("Finished FlattenDeltaTable for source and target");
                ...
                TraceManager.TraceInformation("Edit/Edit conflict detection complete");
            ...
            bool getSourceSideCopyInMigrationInstructionTable)
        {
            ...
            do
            {
                changeGroups = getSourceSideCopyInMigrationInstructionTable ? 
                    TargetChangeGroupService.NextMigrationInstructionTablePage(pageNumber++, pageSize, true, false)
                    : TargetChangeGroupService.NextDeltaTablePage(pageNumber++, pageSize, false);
                foreach (ChangeGroup changeGroupEntry in changeGroups)
                {
                    TraceManager.TraceInformation(String.Format("FlattenDeltaTable processing Change Group {0}; Name: {1}; IsForcedSync: {2}",
                        changeGroupEntry.ChangeGroupId, changeGroupEntry.Name, changeGroupEntry.IsForcedSync));
                    ...
                    foreach (MigrationAction action in changeGroupEntry.Actions)
                    {
                        ...
                        else if (action.Action == WellKnownChangeActionId.Edit)
                        {
                            ...
                            if (changeGroupEntry.IsForcedSync)
                            {
                                // In the ForceSync case, clear out all conflicts and backlogged change groups in the pipeline
                                ConflictManager.ResolveExistingConflictsAndSkipBackloggedItems(Configuration.SessionUniqueIdGuid, sourceId, targetId, sourceSideItemId, targetWorkItemId);
                                continue;
                            }
                            ...
                            if (changeGroupEntry.ContainsBackloggedAction)
                            {
                                // the change group has already been placed in the backlog
                                continue;
                            }
