...
namespace Microsoft.TeamFoundation.Migration.Toolkit
{
    public class SyncOrchestrator
    {
        ...
        private ConflictsSyncOrchOptions m_conflictSyncOption = ConflictsSyncOrchOptions.PauseConflictedSession;
        ...
        {
            TraceManager.TraceInformation(String.Format(MigrationToolkitResources.LogVersion, AssemblyVersionInfo.VersionString));
        ...
        /// <summary>
        /// This method initializes all the session (group) sync state machine to the default state
        /// unless the session was previously in the PausedByConflict state in which case that state is maintained
        /// </summary>
        private void InitializeSyncStates()
        {
            ...
            {
                sessionWorker.InitializeSyncState();
            }
        }
                ...
                try
                {
                    ...
                    switch (m_processingCommand.Value)
                    {
                        ...
                        case PipelineSyncCommand.PAUSE_FOR_CONFLICT:
                            // Should only apply to session, not session group
                            break;
        ...
        void ConflictUnresolvedEventHandler(object sender, ConflictUnresolvedEventArgs e)
        {
            ...
            switch (conflictUnresolvedSyncOrchOption)
            {
                ...
                case ConflictsSyncOrchOptions.PauseConflictedSession:
                    foreach (SessionWorker worker in m_sessionWorkers.Values)
                    {
                        if (worker.Thread == conflictedThread)
                        {
                            TraceManager.TraceInformation("Will pause session {0} due to conflict (resolve all conflicts to resume)", worker.SessionId.ToString());
                            worker.PauseForConflict();
                            break;
                        }
                    }
                    break;
        ...
        private SessionWorker ConstructSessionPipeline(BM.Session config, int sessionIndex, LinkEngine linkEngine)
        {
            ...
            leftContainer.AddService(typeof(IForceSyncItemService), analysisEngine[leftMigrationSourceId].GetService(typeof(IForceSyncItemService)));
            ...
            rightContainer.AddService(typeof(IForceSyncItemService), analysisEngine[rightMigrationSourceId].GetService(typeof(IForceSyncItemService)));
            ...
            IConflictAnalysisService conflictAnalysisService)
        {
            ...
            IForceSyncItemService leftForceSyncItemService = new ForceSyncItemService();
            leftForceSyncItemService.SessionId = sessionUniqueId;
            leftForceSyncItemService.MigrationSourceid = leftSourceId;
            analysisEngine[leftSourceId].AddService(typeof(IForceSyncItemService), leftForceSyncItemService); 
            ...
            IForceSyncItemService rightForceSyncItemService = new ForceSyncItemService();
            rightForceSyncItemService.SessionId = sessionUniqueId;
            rightForceSyncItemService.MigrationSourceid = rightSourceId;
            analysisEngine[rightSourceId].AddService(typeof(IForceSyncItemService), rightForceSyncItemService);
