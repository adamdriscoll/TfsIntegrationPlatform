        ...
        public ReadOnlyCollection<NetworkCredential> GetCredentialsForMigrationSource(Guid migrationSourceId)
        {
            lock (m_configLock)
            {
                ...
                if (IsMigrationSourceConfiguredToUseStoredCredentials(migrationSourceId))
                {
                    ...
                    try
                    {
                        string normalizeCredentialsTargetName = NormalizeCredentialsTargetName(migrationSource.StoredCredential.CredentialString);

                        TraceManager.TraceInformation("CredentialManagementService: Attempting to get credentials using normalized credential string: {0}", normalizeCredentialsTargetName);

                        // 1. try searching by the raw credential string first
                        retVal = this.GetCredentials(normalizeCredentialsTargetName);

                        if (retVal.Count == 0)
                        {
                            TraceManager.TraceInformation("CredentialManagementService: First attempt failed; now attempting to get credentials via Uri: {0}", migrationSource.StoredCredential.CredentialString);
                            ...
                            if (retVal.Count > 0)
                            {
                                TraceManager.TraceInformation("CredentialManagementService: Successfully retrieved credentials");
                            }
                        }
                    }
