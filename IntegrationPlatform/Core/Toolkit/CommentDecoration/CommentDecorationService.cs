...
namespace Microsoft.TeamFoundation.Migration.Toolkit
{
    internal class CommentDecorationService : ICommentDecorationService
    {
        ...
        {
            return GetChangeGroupCommentSuffix(id, DateTime.MinValue);
        }
        ...
        /// <param name="id">An Id that identifies corresponding item on the peer server</param>
        /// <param name="creationDateUtc">The creation date of the changeset that is getting migrated</param>
        /// <returns></returns>
        public string GetChangeGroupCommentSuffix(string id, DateTime creationDateUtc)
        {
            ...
            else
            {
                ...
                // The comparison based only on the Year is because the DateTime value seems to lose some precision when it is stored and retrieved from
                // the TFS_IntegrationDatabase
                string creationTimeString = (creationDateUtc.Year == DateTime.MinValue.Year || creationDateUtc.Year == DateTime.MaxValue.Year) ? string.Empty : 
                    String.Format(MigrationToolkitResources.VerbosePlatformCommentCreationDate, String.Format("{0:yyyy-MM-dd HH:mm:ss} UTC", creationDateUtc));

                return string.Format(CultureInfo.InvariantCulture, MigrationToolkitResources.VerbosePlatformCommentSuffixFormat,
                      Constants.PlatformCommentSuffixMarker, sourceIdentifier, id, creationTimeString, m_customCommentSuffix, Constants.PlatformCommentSuffixMarkerEnd);
            }
        }
