    ...
    internal class LinkEngine
    {
            ...
            ILinkProvider targetLinkProvider)
        {
            ...
            while (true)
            {
                ...
                {
                    if (linkChangeGroup.IsForcedSync)
                    {
                        // Don't raise a conflict in the force sync case
                        continue;
                    }
            ...
            Guid sourceId)
        {
            try
            {
                ...
                IForceSyncLinkProvider forceSyncLinkProvider = linkProvider as IForceSyncLinkProvider;
                if (forceSyncLinkProvider != null)
                {
                    IForceSyncItemService forceSyncItemService = GetService(sessionId, sourceId, typeof(IForceSyncItemService)) as IForceSyncItemService;
                    ...
                    {
                        GenerateLinkDeltaForForceSync(forceSyncItemService, forceSyncLinkProvider);
                    }
                }
            }
            catch (Exception e)
        ...
        private void GenerateLinkDeltaForForceSync(
            IForceSyncItemService forceSyncItemService,
            IForceSyncLinkProvider forceSyncLinkProvider)
        {
            int batchSize = 100;
            ...
            foreach (string forceSyncItemId in forceSyncItemService.GetItemsForForceSync())
            {
                TraceManager.TraceInformation(String.Format(CultureInfo.InvariantCulture,
                    "Will attempt to force sync links for work item {0} from migration source {1}",
                ...
                {
                    forceSyncLinkProvider.GenerateLinkDeltaForForceSync(forceSyncItemIds);
