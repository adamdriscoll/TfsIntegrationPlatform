...
namespace Tfs2008VCTCAdapter
{
    ...
    public class TfsVCTestCaseAdapter : ITfsVCTestCaseAdapter
    {
        ...
        public int Rollback(int changesetFrom, int changesetTo)
        {
            ...
            string pathToTf = null;
            if (AdapterType == MigrationTestLibrary.AdapterType.TFS2008VC)
            {
                pathToTf = Path.Combine(Environment.GetEnvironmentVariable("VS90COMNTOOLS"), @"..\IDE\TF.exe");
            }
            else if (AdapterType == MigrationTestLibrary.AdapterType.TFS2010VC)
            {
                pathToTf = Path.Combine(Environment.GetEnvironmentVariable("VS100COMNTOOLS"), @"..\IDE\TF.exe");
            }
            else
            {
                Debug.Fail("Invalid adapter type");
                return 0;
            }
